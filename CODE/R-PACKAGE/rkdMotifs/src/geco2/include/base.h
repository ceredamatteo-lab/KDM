...
public:
  ...
  bool operator == (int count);
  gRefCounter & operator ++(int dummy);
  gRefCounter & operator --(int dummy);
};
...
public:
  ...
  gRetrieverImplementation(gSize dimCount);
  gRetrieverImplementation(const gRetrieverImplementation & impl);
  virtual ~gRetrieverImplementation();
  gSize  getDimCount() const;
};
...
//This class is used internally by gArray. you will never instatiate an object of this type
...
template <class T> gSize gArrayInternal<T>::internalCount ( T value,gPos start,gPos end,gSize nacount ) const {
    gPos aend=std::min ( (gPos) (i_length-nacount),end );
    gPos left=start;
    gPos right=aend;
    gPos pos;
    bool found=false;
...
template <class T> void gArrayInternal<T>::internalSearch ( gArrayInternal<gPos> *positions,T value,const gArrayInternal<T> & array,gPos start, gPos end,bool excludeNA) const {
    ...
    gPos aend=std::min( array.i_length,end );
    gPos left=start;
    gPos right=aend;
    gPos pos;
    bool found=false;
...
template <class T> void gArrayInternal<T>::setLength ( gSize length,T fillvalue,bool fillNA ) {
    if ( length!=i_length ) {
        ...
        if ( length>0 ) {
            ...
            if ( i_mem ) {
                memmove ( newmem,i_mem,sizeof ( T ) *std::min ( i_length,length ) );
                delete [] i_mem;
            }
            i_mem=newmem;
...
//#pragma omp parallel for
    for(gPos i=start;i<end;i++){
      ret->i_mem[i-start]=i_nans.getValue(i);
    }
    return ret;
}
...
//#pragma omp parallel for private(i)
	 ...
	 ret->i_nans.Or ( 0,array.i_nans,astart,aend );
     } else {
         for ( i=0;i<l1;i+=l2 ) {
//#pragma omp parallel for private(j)
...
//#pragma omp parallel for private(i)
        for ( i=0;i<l1;i++ ) ret->i_mem[i] = tval[i] != aval[i];
	ret->i_nans.Or(0,array.i_nans,astart,astart+l1);
...
template <class T> gArrayInternal<T> * gArrayInternal<T>::getUnique (gPos start,gPos end ) const {
    ...
    if ( cpos<end ) {
        ...
        ret=new gArrayInternal ( npos+1,data,gBitsArray ( npos+1,false ),false,i_sorted );
        delete [] data;
    } else {
            ...
            if ( values.i_sorted ) {
                for ( gPos i=start;i<end;i++ ) {
                    if ( !i_nans.getValue ( i ) ) {
                        gArrayInternal<gPos> * pos=values.find ( i_mem[i],vstart,vend,true );
                ...
                for ( gPos i=start;i<end;i++ ) {
                    if ( !i_nans.getValue ( i ) ) {
                        gArrayInternal<gPos> * pos=ovalues.find ( i_mem[i],0,ovalues.i_length,true );
                        for ( gPos j=0;j<pos->getSize();j++ ) {
...
template <class T> gArrayInternal<T> * gArrayInternal<T>::getWinSum ( gArrayInternal<T> * res,  gSize winlength,gSize pos,gPos start,gPos end,bool skipnan ) const {
  ...
  if(skipnan){
    ...
    gArrayInternal<T> * mysum= new gArrayInternal<T> ( 1,0,true );
    res->i_nans.setAll();
    mysum->setValue(0,0,true,0,false);
    gPos cpos;
    for(cpos=start; cpos< ( end-winlength+1 ); cpos++ ) {
      while( mysum->isNA(0) && cpos < ( end-winlength+1 ) ){
	getSum (mysum,cpos,cpos+winlength,false);
	//res->setValue ( cpos+pos, mysum->at ( 0 ), mysum->isNA ( 0 ),0,false );
	cpos++;
      }
      res->setValue (cpos-1+pos, mysum->at ( 0 ), mysum->isNA ( 0 ),0,false );
      if(cpos < ( end-winlength+1 ) ){
	mysum->setValue(0,mysum->at(0)-this->at(cpos-1)+this->at(cpos+winlength-1),this->isNA(cpos+winlength-1),0,false);
      }
    }
    res->setValue (cpos-1+pos, mysum->at ( 0 ), mysum->isNA ( 0 ),0,false );
    delete mysum;
...
template <class T> gArrayInternal<gScore> * gArrayInternal<T>::getWinStdDev ( gArrayInternal<gScore> * res, gSize winlength,gSize pos,gPos start,gPos end,bool skipnan,bool unbiased ) const {
    bool ok;
    gPos first=start,last=start;
    gScore sum=0,ssum=0,n= ( unbiased ) ? ( -1 ) : ( 0 );
