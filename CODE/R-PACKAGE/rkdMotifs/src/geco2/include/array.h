...
template <class T> class gArray {
    template <class U> friend class gArrayAccessor;
...
protected:
    ...
    gPos getInternalStart() const;
    gPos getInternalEnd() const;
    void setSorted(bool sorted);
public:
    /** @name Public constructors & destructors */
    //@{
    gArray();
    gArray(const T* data,gSize length,const gArray<gPos> & NaPositions=gArray<gPos>(),bool sorted=false);
    template < class C > gArray(const std::vector<C> data,const gArray<gPos> & NaPositions=gArray<gPos>(),bool sorted=false);
    ...
    void setValues(const gArray< gPos >& positions, const gArray< T >& values, T fillvalue = (T) 0, bool fillNA=true);
    ...
    void setNoNa();
    void setAllNa();
    void setNA(const gArray<gPos> & positions,T fillValue=0,bool fillNA=true);
    template < class C > void NACopy(const gArray<C> & array);
    ...
    template <class C> operator std::vector<C> () const;
    //@}
...
/** @brief Array accessor template class
 *
 ...
 */
template <class T> class gArrayAccessor {
protected:
 ...
 */
template <class T> class gArrayRetrieverImplementation:public gRetrieverImplementation,public gArrayAccessor<T> {
  friend class gArrayRetriever<T>;
private:

public:
    gArrayRetrieverImplementation(gSize dimCount);
    gArrayRetrieverImplementation(const gArrayRetrieverImplementation<T> & impl);
...
public:
    ...
    gMatrix<T> getRows(const gArray<gPos> & rows) const;
    gMatrix<T> getRows(gPos start,gPos end) const;
    gMatrix<T> getColumns(const gArray<gPos> & cols) const;

    T operator () ( gPos row,gPos col ) const;

    template <class C> operator gMatrix<C> () const;
...
public:
    ...
    std::vector<gString> &  split(gChar splitChar,std::vector<gString> & parts, bool skipEmpty=false) const;
 ...
 */
template <class T> class gPositionInterval:private geco::gArray<T> {
public:
    /** @name Constructors and assignment */
    //@{
    gPositionInterval(T start =  0, T end =  0, bool validStart = true, bool validEnd = true);
    gPositionInterval(const geco::gArray<T> & start,const geco::gArray<T> & end);
    gPositionInterval(const geco::gArray<T> & positions);
    ...
    bool validEnd() const;
    geco::gArray<T> getPositions() const;
    gArray<bool> contains(const geco::gArray<T> & positions) const;
    bool containsInterval(const gPositionInterval<T> & interval) const;
...
gArray<gPos> randomSample(gSize n, gPos min, gPos max);
...
/*  
    ...
    return ret;
*/
  const T * data=array.getConstData();
  gArrayAccessor<gBool> acc;
  T * rvals=new T[array.getSize()];
  gArray<gBool> aN=array.isNA();
  gBool * NAs= new gBool[array.getSize()];
  memset(NAs,0,sizeof(gBool)*array.getSize());
  for (gSize i=0;i<array.getSize();i++) {
    try {
      rvals[i]=function(data[i]);
      if (rvals[i]!=rvals[i]) NAs[i]=1;
    } catch (...) {
      rvals[i]=0;
      NAs[i]=1;
    }
  ...
  gArray<gBool> NAz;
  NAz.ownData(array.getSize(),NAs);
  gArray<T> ret;
  ret.ownData(array.getSize(),rvals,which(aN||NAz));
  return ret;
}

/** @brief Template gArray output stream operator
 *
 * Template gArray simple stream output
 * @param out the output stream
 * @param arr the array to be streamed out
 * @return a reference to the output stream (for chainig outputs)
 * @ingroup arrays
 */
template <class T> std::ostream & operator << (std::ostream & out, const gArray<T> & arr){
  gShortUnsigned precision=out.precision();
  //out.precision(precision);
  for(gSize r=0;r<arr.getSize();r++){
    if(arr.isNA(r)){
      out << std::setw(precision+2) << "*";
    }else{
      out << std::setw(precision+2) << arr[r];
    }
  }
  //out << std::endl;
  return out;
...
/** @brief Template gMatrix output stream operator
 *
 * Template gMatrix simple stream output
 * @param out the output stream
 * @param mat the matrix to be streamed out
 * @return a reference to the output stream (for chainig outputs)
 * @ingroup arrays
 */
template <class T> std::ostream & operator << (std::ostream & out, const gMatrix<T> & mat){
  gShortUnsigned precision=out.precision();
  //out.precision(precision);
  for(gSize r=0;r<mat.getRowsNum();r++){
    for(gSize c=0; c<mat.getColsNum();c++){
      if(mat.isNA(r,c)){
        out << std::setw(precision+2) << "*";
      }else{
        out << std::setw(precision+2) << mat(r,c);
      }
    }
    out << std::endl;
  }
...
/** @brief Template gString to gArray conversion
 *
 * Template gString to gArray conversion.
 * If a separator is specified the array will contain all the values in the string.
 * If no separaor is specified (0) whatever the string contains only the first number is returned
 * Values failing to convert are returned as NA.
 * @param str the string to be converted
 * @param sep an optional separator character (default ascii code 0: to signal no separator used)
 * @return An array containing the converted values
 * @ingroup arrays
 */
template <class T> gArray<T> gStringToNumArray(const gString & str,char sep=0){
  std::vector<gString> spline;
  str.split(sep,spline);
  gArray<T> num(0,spline.size(),true);
  char * endptr;
  for(gSize i=0;i<spline.size();i++){
    std::string s=spline[i];
    long double val=strtold(s.c_str(),&endptr);
    if((!errno)&&(endptr>s.c_str())){
      num.setValue(i,(T)val,false);
    }
  }
  return num;
}
...
/** @brief std::vector of arbitrary data constructor
 *
 * This constructor instantiates an array initialized using the values prvided in a std::vector.
 * Positions provided in another std:vector are markedas NA.
 *
 * @param data a const vector<C> used for initialization: its values are casted to T.
 * @param NaPositions a vector<gPos> containing the positions of NA values (defaults to an empty vector)
 * @param sorted to be set if data contains sorted values (defaults to false)
 */

template <class T> template <class C> gArray<T>::gArray(const std::vector<C> data,const gArray<gPos> & NaPositions,bool sorted){
 i_array=NULL;
 i_start=0;
 i_end=0;
 *this=gArray<T>(0,data.size(),false);
 for(gPos i=0;i<data.size();i++){
   setValue(i,(T) data[i],false);
 }
 setNA(NaPositions);
}
...
template <class T> bool gArray<T>::isSorted() const{
  if(i_array){
    return i_array->i_sorted;
  }else{
    return false;
  }
}
...
template <class T> void gArray<T>::setValue ( gPos pos, T value,bool isna,T fillvalue,bool fillNA ) {
    ...
    i_end=std::max ( i_end,i_start+pos+1 );
}
...
template <class T> void gArray<T>::setValue ( gPos destpos,const gArray<T> array,gPos sourcepos,T fillvalue,bool fillNA ) {
    ...
    i_end=std::max ( i_end,i_start+destpos+1 );
}
...
template <class T> void gArray<T>::setValues ( const gArray<gPos> & positions, const gArray<T> & values, T fillvalue,bool fillNA ) {
    if (positions.getSize()>0) {
        ...
        i_end=std::max(i_end,i_start+mpos+1);
    }
}
...
/** @brief Set values to NA at spcified positions
 *
 * This method set NA flags to true at specified positions
 *
 * @param positions: array containing positions to be set to NA
 */
template <class T> void gArray<T>::setNA(const gArray<gPos> & positions,T fillvalue,bool fillNA){
  for(gPos i=0;i<positions.getSize();i++){
    setValue(positions[i],(*this)[positions[i]],true,fillvalue,fillNA);
  }
}

/** @brief Copies complete NA status from another array
 *
 * This method set NA flags to true at specified positions
 *
 * @param array: array from which NA status shoyld be copied
 */
template <class T> template <class C> void gArray<T>::NACopy(const gArray<C> & array){
  if(array.getSize()==getSize()){
    i_array->i_nans.copyValues(i_start,array.i_array->i_nans,array.i_start,array.i_end);
  }else throw gException("Attempt to copy NA status between incompatible arrays");
}
...
/** @brief Array data const access
 *
 * Returns a const pointer to array data
 * @return T *
 */
template <class T> const T * gArray<T>::getConstData() const{
    T* ret=NULL;
...
template <class T> gArray<T> & gArray<T>::insert ( gPos pos,const gArray<T> & array,gPos astart,gPos aend ) {
    ...
    gSize cend=( ( aend==0 ) ? ( array.getSize() ) : ( aend ) );
    i_array->selfInsert ( i_start+pos,*array.i_array,array.i_start+astart,array.i_start+cend );
    i_end+=cend-astart;
    return *this;
}
...
/** @brief Vector template type casting operator
 *
 * Allow for array casting to a vector<C>
 * containing original values (C) values.
 * @tparam the type to which array elements must be cast
 * @return a vector<C> object
 */
template <class T> template <class C> gArray<T>::operator std::vector<C> () const{
//   vector<C> ret(getSize());
//   if(i_array){
//     for(gPos i=0;i<getSize();i++){
//       ret[i]=(C) (*this)[i];
//     }
//   }
  std::vector<C> ret;
  ret.assign(getData()+i_start,getData()+i_end);
  return ret;
}
...
template <class T> gPos gArray<T>::getInternalStart() const{
  return i_start;
}

template <class T> gPos gArray<T>::getInternalEnd() const{
  return i_end;
}

template <class T> void gArray<T>::setSorted(bool sorted){
  i_array->i_sorted=sorted;
}
//--------------------------------------------------------------------------


//--------------------------------------------------------------------------
// template class gArrayAccessor implementation
//--------------------------------------------------------------------------
template <class T>  gArrayAccessor<T>::gArrayAccessor(){
}

template <class T> template<class C> const C * gArrayAccessor<T>::getArrayDataPtr ( const gArray<C> & array ) const {
    array.setSorted(false);
    return array.getData();
}

template <class T> template <class C> C * gArrayAccessor<T>::getArrayDataPtr(gArray<C> & array) const{
  array.setSorted(false);
  return array.getData();
}

template <class T> unsigned gArrayAccessor<T>::getArrayReferenceCount(gArray<T> & array) const {
    unsigned ret=0;
...
//--------------------------------------------------------------------------
template <class T>  gArrayRetrieverImplementation<T>::gArrayRetrieverImplementation(gSize dimCount):gRetrieverImplementation(dimCount),gArrayAccessor<T>(){

}

template <class T>  gArrayRetrieverImplementation<T>::gArrayRetrieverImplementation(const gArrayRetrieverImplementation<T> & impl):gRetrieverImplementation(impl),gArrayAccessor<T>(){
...
template <class T> gSize gMatrix<T>::size1() const{
  return nrows;
}
...
template <class T> gSize gMatrix<T>::size2() const{
    return ncols;
}
 ...
 * @param rows an garray<C> containing the row numbers of the elements to retrieve
 * @param cols an garray<C> containing the column numbers of the elements to retrieve
...
template <class T> template <class C> gArray<T> gMatrix<T>::operator () (const gArray<C> & rows, const gArray<C> & cols) const {
 if(rows.getSize()==cols.getSize()){
   return (*(gArray<T>*)this)(rows,cols,ncols);
 }else throw gException("gMatrix() multiple access operator: rows and columns must have the same number of elements");
}
 ...
 *
 */
template <class T> template <class C> gMatrix<T>::operator gMatrix<C> () const{
  return gMatrix<C>(nrows,ncols,(gArray<C>) *this);
}
...
/** @brief Row access
 *
 * Returns an array contining the values of the specified row
...
/** @brief Column access
 *
 * Returns an array contining the values of the specified clumn
...
template <class T> gArray<T> gMatrix<T>::getCol ( gPos colnum ) const {
  gArray<T> ret;
  if(nrows>0){
    ret=( *this ) [getArray<gPos> ( colnum,this->getSize()-1,ncols ) ];
  }
  return ret;
}

/** @brief Sub matrix access by rows
 *
 * Returns a matrix constituted by the specified rows numbers
 * @param rows an array containing the rows indexes to retrieve
 * @return a newly allocated matrix
 */
template <class T> gMatrix<T> gMatrix<T>::getRows ( const gArray<gPos> & rows ) const {
  gMatrix<T> ret(rows.getSize(),ncols,0,false);
  if(ncols>0){
    for(gSize i=0;i<rows.getSize();i++){
      ret.setRow(i,getRow(rows[i]));
    }
  }
  return ret;
}
...
/** @brief Sub matrix access by rows
 *
 * Returns a contiguous range of rows
 * @param start the range start
 * @param end the range end
 * @return a newly allocated matrix
 */
template <class T> gMatrix<T> gMatrix<T>::getRows(gPos start,gPos end) const{
  return gMatrix<T>(end-start,ncols,gArray<T>(*this,start*ncols,end*ncols));
}

/** @brief Sub matrix access by columns
 *
 * Returns a matrix constituted by the specified cols
 * @param cols an array containing the columns index tio retrieve
 * @return a newly allocated matrix
 */
template <class T> gMatrix<T> gMatrix<T>::getColumns ( const gArray<gPos> & cols ) const {
  gMatrix<T> ret(nrows,cols.getSize(),0,false);
  if(nrows>0){
    for(gSize i=0;i<cols.getSize();i++){
      ret.setCol(i,getCol(cols[i]));
    }
  }
  return ret;
}
...
template <class T> gPositionInterval<T>::gPositionInterval(T start,T end,bool validStart,bool validEnd):gArray<T>(0,2,false,false) {
    this->setValue(0,start,!validStart);
    this->setValue(1,end,!validEnd);
}


/** @brief Constructor from positions (arrays)
 *
 * Instatiate a new gInterval from absolute boundaries
 * @param start the absolute interval-start position (0 based,default 0) first position of the array will be used.
 * @param end the absolute interval-end position (1 based,default 0) first position of the array will be used.
 */
template <class T> gPositionInterval<T>::gPositionInterval(const geco::gArray<T> & start,const geco::gArray<T> & end):geco::gArray<T>(0,2,false,false) {
    this->setValue(0,start[0],start.isNA(0));
    this->setValue(1,end[0],end.isNA(0));
}
...
*/
template <class T> gPositionInterval<T>::gPositionInterval(const geco::gArray<T> & positions):geco::gArray<T>(0,2,false,false)  {
    this->setValue(0,positions[0],positions.isNA(0));
    this->setValue(1,positions[1]+1,positions.isNA(1));
}
...
template <class T> gPositionInterval<T> & gPositionInterval<T>::operator = (const gPositionInterval<T> & positionInterval) {
    this->setValue(0,positionInterval[0],positionInterval.isNA(0));
    this->setValue(1,positionInterval[1],positionInterval.isNA(1));
    return *this;
}
...
*/
template <class T> geco::gArray<T> gPositionInterval<T>::getPositions() const {
    geco::gArray<T> ret(*this);
...
*/
template <class T> gArray< bool > geco::gPositionInterval< T >::contains(const geco::gArray< T >& positions) const
{
    return (positions>=(*this)[0])&&(positions<(*this)[1]);
}

template <class T> bool gPositionInterval<T>::containsInterval(const gPositionInterval<T> & interval) const{
//   return (interval[0]>=(*this)[0]) &&
//          (interval[1]<=(*this)[1]) &&
//          (!gArray<T>::isNA(0)) &&
//          (!gArray<T>::isNA(1)) &&
//          (!(gArray<T>)interval).isNA(0) &&
//          (!(gArray<T>)interval).isNA(1);
  return (interval[0]>=(*this)[0]) && (interval[1]<=(*this)[1]) && validStart() && validEnd() && interval.validStart() && interval.validEnd();
}
