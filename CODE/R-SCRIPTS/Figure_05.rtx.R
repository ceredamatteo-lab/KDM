...
get_binom_test=function(data,condition,adjust=TRUE){
	...
	if(length(unique(data$variable))==3){r=unique(data$variable)[1]}
...
### BEGIN TF_BENCHMARK CLASSIFICAZIONE
load(paste0(data_folder,"classification_MEME_TF-ENCODE.Rdata"))
load(paste0(data_folder,"classification_KDM_TF-ENCODE.Rdata"))
result=data.frame(KDM_KDMref_KDMMATCH=class_sel_kdm_wall_kdmtom$kdm_cat,MEME_MEMEref_TOMTOM=class_sel_meme_memeall_tomtom$meme_cat)

p3<-plotClassification(result,"KDM",7)
pdf(paste0(data_folder,"Fig5_TF_ENCODE_classification.pdf"),height = 8,width = 10)
ggarrange(plotlist=p3,nrow=1)
dev.off()

### END TF_BENCHMARK CLASSIFICAZIONE

### BEGIN  RBP_BENCHMARK CLASSIFICAZIONE
load(paste0(data_folder,"classification_STREME_RBP-ENCODE.Rdata"))[,2:1]
results=tt3
colnames(result)<-c("KDM_KDMref_KDMMATCH","STREME_STREMEref_TOMTOM") 
p3<-plotClassification(result,"KDM",7)
pdf(paste0(data_folder,"Fig5_RBP_ENCODE_classification.pdf"),height = 8,width = 10)
ggarrange(plotlist=p3,nrow=1)
dev.off()

### END RBP_BENCHMARK CLASSIFICAZIONE