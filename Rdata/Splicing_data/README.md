...
03. **RBPname_CellLine_Genome_ridge.rds** : this Rdata objects is a list that contains multiple object genreated through the analysis
  03.1 *coeff* : a dataframe with the glm coefficents of each MRMD in each subregion   
  03.2 *sel* : MRMDs with at least one subregion in which the overrepresentation is significant
  03.3 *profile* : score of each MRMD in each position of each subregion on both altered and control regions
  03.4 *test*: overrepresentation results of each MRMD in each subregion
  03.5 *pwm*s: MRMD to PWM convertion of each MRMD
  03.6 *kdmtom*: results for the comparison betweeen each MRMDs and mCross PWMs converted into MRMD
