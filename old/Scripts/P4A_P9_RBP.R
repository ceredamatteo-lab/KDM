...
pwms<-kdmLoadPWMSet(queryfile)
train2=train0
train2$start=train2$start-expand-half_pwm-max(hws)
train2$end=train2$end+expand+half_pwm+max(hws)
seqs2<-kdmGetSequence(train2,file.name="/adat/database/2bit/hg38.2bit")
spos2<-seqs2[which(trainLabels==1)]
prof<-pwmProfile(spos2,queryfile,sel,pwms[pp])
R1<-pwmWin(prof$score[[1]],hws[1])
R2<-pwmWin(prof$score[[1]],hws[2])
...
tmp=list(
  ...
  data.frame(Pos=sel_fimo$Pos[order(sel_fimo$Pos)],Score = R1[ss,][(half_pwm+max(hws)+1):(ncol(R1)-half_pwm-max(hws))],Method = "PWM + window: 13"),
  data.frame(Pos=sel_fimo$Pos[order(sel_fimo$Pos)],Score = R2[ss,][(half_pwm+max(hws)+1):(ncol(R2)-half_pwm-max(hws))],Method = "PWM + window: 71")
)
...
lev=c("PWM",
paste0("PWM + window: ",hws[1]*2+1),paste0("KDM: win=", hws[1]*2+1),
paste0("PWM + window: ",hws[2]*2+1),paste0("KDM: win=", hws[2]*2+1))
final=do.call(rbind, tmp) %>%mutate(Method = factor(Method, levels = lev))
...
pA=ggplot(final,aes(x=Pos,y=Score))+geom_line()+
  ...
  theme_bw()+xlab("Score on  single sequence")+
  theme( plot.margin = unit(c(1, 2, 1, 1), "cm"),panel.grid=element_blank())

pdf(paste0(plot_folder,"P4A_RBP.pdf"),height = 7,width = 8)
pA
dev.off()
